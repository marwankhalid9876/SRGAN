...
#this function is added to preprocess the range of the data
def scale(lr_img, hr_img):
    lr_img = tf.divide(
        tf.subtract(
            lr_img,
            tf.reduce_min(lr_img)
        ),
        tf.subtract(
            tf.reduce_max(lr_img),
            tf.reduce_min(lr_img)
        )
    )
    hr_img = tf.divide(
        tf.subtract(
            hr_img,
            tf.reduce_min(hr_img)
        ) ,
        tf.subtract(
            tf.reduce_max(hr_img),
            tf.reduce_min(hr_img)
        )
    )
    hr_img = hr_img * 2 - 1
...
def download_archive(file, target_dir, extract=True):
    source_url = f'https://cv.snu.ac.kr/research/EDSR/Flickr2K.tar{file}'
